...
func (p *Pipe) Execute(){
	...
	p.MReg = AccessMemoryReg{
		PipeReg{ &NormalStatus{}, false, 0},
		p.EReg.MicroInst, isPredictError, ValE, p.EReg.ValRs2,
		p.EReg.DstE, p.EReg.DstM, p.EReg.UnselectedPC,
		}
	p.e_valE = ValE
	p.EReg.Status =  &NormalStatus{}
}
...
func (p *Pipe) Decode(){
	...
	SelectRegVal := func(index uint8) uint64 {
		if index==0 {return 0}
		val := p.R.Load(index)
		if p.M_dstM == index {val = p.m_valM}
		if p.M_dstE == index {val = p.M_valE}
		if p.E_dstE == index {val = p.e_valE}
		return val
	}
...
func (p *Pipe) Fetch() (*MicroAction, error) {
	...
	SelectPC := func() uint64{
		rv := p.PC
		if p.M_IsIndirectJump {rv = p.M_valE}
		if p.M_IsBranch && p.M_IsPredictError {rv = p.M_UnselectedPC}
		return rv
	}
	...
	var Imm int32 = 0
	if val:=reflect.ValueOf(microInst.Inst).FieldByName("Rs1"); val.IsValid(){rs1=uint8(val.Uint())}
	if val:=reflect.ValueOf(microInst.Inst).FieldByName("Rs2"); val.IsValid(){rs2=uint8(val.Uint())}
	if val:=reflect.ValueOf(microInst.Inst).FieldByName("Imm"); val.IsValid(){Imm=int32(val.Int())}

	p.DReg = DecodeReg{
		PipeReg{ &NormalStatus{}, false, 0},
		*microInst, rs1, rs2, Imm,
		microInst.dstE, microInst.dstM, microInst.ValCFunction(pc, Imm), microInst.NegativeOptionPC(pc, Imm),
	}
